"""
	Inference Stage 2 - Using training checkpoint
"""
...
class InferenceAgent:
	...
	def load_weight(self, checkpoint_path: str, rank: int) -> None:
		# Load original checkpoint for most components
		original_ckpt_path = self.opt.ckpt_path
		original_state_dict = torch.load(original_ckpt_path, map_location='cpu', weights_only=True)

		# Load training checkpoint for FMT (Flow Matching Transformer)
		fmt_checkpoint = torch.load(self.opt.fmt_ckpt_path, map_location='cpu', weights_only=True)

		# Extract FMT state dict from training checkpoint
		if 'model_state_dict' in fmt_checkpoint:
			fmt_state_dict = fmt_checkpoint['model_state_dict']
			print(f"Loading FMT from training checkpoint at step {fmt_checkpoint.get('global_step', 'unknown')}")
		else:
			# Fallback to direct state dict if not a training checkpoint
			fmt_state_dict = fmt_checkpoint

		with torch.no_grad():
			for model_name, model_param in self.G.named_parameters():
				# Use FMT weights from training checkpoint for fmt.* parameters
				if model_name.startswith('fmt.'):
					# Remove 'fmt.' prefix to match training checkpoint parameter names
					fmt_param_name = model_name[4:]  # Remove 'fmt.' prefix
					if fmt_param_name in fmt_state_dict:
						model_param.copy_(fmt_state_dict[fmt_param_name].to(rank))
						print(f"Loaded FMT parameter: {model_name} <- {fmt_param_name}")
					else:
						print(f"! Warning: FMT parameter {fmt_param_name} not found in training checkpoint")
				# Use original weights for other components
				elif model_name in original_state_dict:
					model_param.copy_(original_state_dict[model_name].to(rank))
				elif "wav2vec2" in model_name:
					pass  # Skip wav2vec2 parameters
				else:
					print(f"! Warning; {model_name} not found in any state_dict.")

		del original_state_dict, fmt_state_dict, fmt_checkpoint
...
class InferenceOptions(BaseOptions):
	...
	def initialize(self, parser):
		...
		parser.add_argument('--fmt_ckpt_path',
				default="./training/checkpoints/step_10000.pt", type=str, help='fmt checkpoint path')
